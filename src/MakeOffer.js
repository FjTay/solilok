...
const MakeOffer = () => {
  ...
  const getCars = async () => {
    ...
    const cars = query(
      carsQuery,
      where("user_id", "==", userInfo.auth.currentUser.uid)
    );
  ...
  useEffect(() => {
    ...
    let start = `${year}-${month < 10 ? `0${month}` : month}-${
      day < 10 ? `0${day}` : day
    }`;
    let end = `${year}-${month < 10 ? `0${month}` : month}-${
  ...
  return (
    <>
          ...
          <>
            ...
            {cars.map((car) => (
              <div
                onClick={() => {
                  setCurrentCar(
                    cars.filter((subcar) => subcar.carID === car.carID)
                  );
                }}
              >
                {car.model} - {car.brand}
      ...
      <div>
        {currentCar.length &&
          `You have chosen : ${currentCar[0].model} - ${currentCar[0].brand}`}
      </div>
      ...
      <input
        ...
        onChange={(e) => {
          ...
          console.log(
            new Date(date.start).getTime() < new Date(date.end).getTime()
          );
        }}
      ></input>
