...
const Offers = () => {
    ...
    const [date, setDate] = useState({start: false, end: false , tosay : false})
    ...
    useEffect(() => {
        console.log(Date.parse(date.end) > Date.parse(date.start))
    }, [date])
    ...
    return (
      ...
      <div className="page page-comments">
        {offers.length && 
            <>
                <div>Offers</div>
                {offers.filter(offer => Date.parse(date.start) >= Date.parse(offer.start) && Date.parse(date.end) <= Date.parse(offer.end)).map(offer =>
                    <div>
